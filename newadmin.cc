...
//  dealer_events de;
  ...
  // TODO: why is this sleep needed?  Maybe the IO Loop is somehow not ready to
  // accept the connect request?
  sleep(2);

  g_client->connect( "127.0.0.1", 55555, connect_cb_2, nullptr);
  // TODO: I think it might be a fatal error if we try to connect to multiple Routers

  // TODO: I have no way to wait for a connect to succeed?
  sleep(3);
  ...
  while (1)   sleep(1);
  g_client.reset(); // delete before logger
  delete logger;
  return 0;
}
