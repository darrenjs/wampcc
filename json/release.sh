...
# Determine version
v=$(grep AC_INIT configure.ac | sed -r 's/.*\[jalson\], \[([0-9.]*)\],.*/\1/')
...
autoreconf -fi
./configure --with-jansson=/home/darrens/opt/jansson-2.7
...
# for s in gz bz2; do
#     gpg --detach-sign --armor jalson-$v.tar.$s
# done

# # Build documentation
# make html
# mv doc/_build/html jalson-$v-doc

# # Make and sign documentation tarballs
# for s in gz bz2; do
#     [ $s = gz ] && compress=gzip
#     [ $s = bz2 ] && compress=bzip2
#     tar cf - jalson-$v-doc | $compress -9 -c > jalson-$v-doc.tar.$s
#     gpg --detach-sign --armor jalson-$v-doc.tar.$s
# done
