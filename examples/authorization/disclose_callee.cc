...
void rpc_called(wamp_session& ws, invocation_info invoke)
{
  ...
  //std::cout << "details: " << invoke.details << std::endl;
  std::string user = "unknown";
  if( invoke.details.find("caller_authid") != invoke.details.end() )
    user = invoke.details["caller_authid"].as_string();

  ws.yield(invoke.request_id, json_array({"hello", user}));
}
...
int main(int argc, char** argv)
{
  try
  {
    ...
    session->provide("greeting.personalized", { },
