...
int main(int argc, char** argv)
{
  try
  {
    ...
    /* Create the wampcc kernel, which provides event and IO threads. This
     * kernel is enabled to support SSL, which then allows the use of
     * ssl_socket.  The ssl_socket transport must be used for connecting to, or
     * providing, secure websocket. */

    wampcc::config config;
    config.ssl.enable = true;
    std::unique_ptr<kernel> the_kernel(new kernel(config, logger::console()));

    /* Create the SSL socket and attempt to connect.  The peer must be expecting
     * an SSL connection, i.e., it must be providing a WSS service. Note, that
     * after this stage, the remaining steps of this example are the same as
     * basic_callee.cc */

    std::unique_ptr<tcp_socket> sock (new ssl_socket(the_kernel.get()));
